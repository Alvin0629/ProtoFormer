...
class Logger:
    ...
    def _print_training_status(self):
        ...
        data = [self.running_loss[k]/self.cfg.sum_freq for k in self.running_loss.keys()]
        ks = [k for k in self.running_loss.keys()]
        training_str = "[{:6d}, {}] ".format(self.total_steps+1, self.scheduler.get_last_lr())
        metrics_str = (f"{ks[0]}: {data[0]:6.4f}, {ks[1]}: {data[1]:6.4f}, {ks[2]}: {data[2]:6.4f}, {ks[3]}: {data[3]:6.4f}")
