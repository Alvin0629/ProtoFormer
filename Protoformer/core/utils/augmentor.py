...
class SparseDepthAugmentor:
    ...
    def __call__(self, img1, img2, flow):
        img1, img2 = self.eraser_transform(img1, img2)  #comments for kitti
        img1, img2, flow = self.spatial_transform(img1, img2, flow)
...
class SparseDepthAugmentor_KITTI:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False):
        # spatial augmentation params
        self.crop_size = crop_size
        self.min_scale = min_scale
    ...
    def spatial_transform(self, img1, img2, flow):
        ...
        margin_top = int(img1.shape[0] - self.crop_size[0])
        margin_left = int((img1.shape[1] - self.crop_size[1]) / 2)

        img1 = img1[margin_top: margin_top + self.crop_size[0],
                  margin_left: margin_left + self.crop_size[1]]
        img2 = img2[margin_top: margin_top + self.crop_size[0],
                  margin_left: margin_left + self.crop_size[1]]
        flow = flow[margin_top: margin_top + self.crop_size[0],
                  margin_left: margin_left + self.crop_size[1]]
