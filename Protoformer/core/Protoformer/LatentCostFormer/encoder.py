...
class MemoryEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.fnet == 'twins':
            self.feat_encoder = twins_svt_encode(pretrained=self.cfg.pretrain)
    ...
    def forward(self, img1, img2, data, context=None):
        ...
        feat_s_enc = self.channel_convertor(feat_s[1])  
        feat_t_enc = self.channel_convertor(feat_t[1]) 

        B, C, H, W = feat_s[1].shape
        size = (H, W)

        cost_volume = self.corr(feat_s_enc, feat_t_enc)
