...
class flow_head(nn.Module):
    def __init__(self, cfg):
        ...
        from .gru_flow import BasicUpdateBlock, GMAUpdateBlock
...
class depth_head(nn.Module):
    def __init__(self, cfg):
        ...
        from .gru_dep import GMAUpdateBlock, SepConvGRU
        ...
        self.convX21 = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=2, padding=0, bias=True),
            torch.nn.Tanh())
        
        self.convX31 = torch.nn.Sequential(
            nn.ReflectionPad2d(1),
            torch.nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3, stride=1, padding=0, bias=True),
            torch.nn.Tanh())
    ...
    def upsample_depth(self, depth, mask):
        ...
        return up_depth.reshape(N, 1, 8 * H, 8 * W)
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        coords0, coords1 = initialize_depth(x2)
        N, fC, fH, fW = x2.shape       #x1: H/4,W/4; x2: H/8,W/8  
        ...
        dispFea = torch.zeros([N, 1, fH, fW], requires_grad=True).to(x1.device)
        net = torch.zeros([N, 256, fH, fW], requires_grad=True).to(x1.device)
        ...
        for idx in range(iters):
            ...
            disp = self.sigmoid(dispFea) #* MAX_DEPTH, if use absolute depth without any inv in loss.py, then multiply MAX_DEPTH to sigmoid output     
##            print('=====disp', disp.shape)
            disp_up = self.upsample_depth(disp, up_mask)
            depth_predictions.append(disp_up)
        
        if self.training:
            return depth_predictions
        else:
            return depth_predictions[-1], coords1-coords0