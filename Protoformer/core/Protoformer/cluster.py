...
class Clustering(nn.Module):
    ...
    def forward(self, x):  # [b,c,w,h]
        ...
        if self.window_w > 1 and self.window_h > 1:
            b, c, w, h = x.shape
            print(x.shape)
            print(self.window_w, self.window_h)
...
class ClusterBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=4.,
                 ...
                 center_w=2, center_h=2, window_w=2, window_h=2, heads=4, head_dim=24, return_center=False,
                 num_clustering=1):

        super().__init__()

        self.norm1 = norm_layer(dim)
        self.token_mixer = Clustering(dim=dim, out_dim=dim, center_w=center_w, center_h=center_h, window_w=window_w,
                                      window_h=window_h, heads=heads, head_dim=head_dim, return_center=return_center,
...
def basic_blocks(dim, index, layers,
                 ...
                 use_layer_scale=True, layer_scale_init_value=1e-5,
                 center_w=5, center_h=5, window_w=5, window_h=5, heads=4, head_dim=24, return_center=False,
                 num_clustering=1):
    blocks = []
